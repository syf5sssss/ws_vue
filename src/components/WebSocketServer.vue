...
function open() {
    const wsUrl = `ws://0.0.0.0:${port.value}`;
    socket = new WebSocket(wsUrl);
    Android.startWebSocketServer(port.value);
    logServerMessage(wsUrl);
}
...
function send() {
    ...
    Android.sendMessageToClients(msg.value);
}
function receiveMessage(message) {
    chatting_ls.value.push({ tag: "accpet", data: message });
    scrollToBottomChatting();
}
...
function ipshow(ipstr) {
    ips.value = ipstr;
}
// 挂载到 window 对象
onMounted(() => {
    window.receiveMessage = receiveMessage;
    window.ipshow = ipshow;
});

// 组件卸载时移除（可选）
onUnmounted(() => {
    delete window.receiveMessage;
    delete window.ipshow;
});
...
<template>
    ...
    <div class="flex items-center p-1">
        <InputText style="width: 100%;" type="text" v-model="port" class="mt-0 ml-0"></InputText>
    </div>
